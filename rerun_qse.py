#!/usr/bin/env python3
"""
rerun_qse.py — PA-DSE+QSE+QSD experiments.

PA-DSE Layer 2 (DFRL) gets two QoR-aware extensions:

1. QSE (QoR-Saturation Extension): Penalty on (dim, value) regions where
   many successes produce few unique (area, latency) points. Defers
   saturated regions, exposing under-explored ones. Fixes best_lat.

2. QSD (QoR-space Diversification): When the global QoR cloud's saturation
   exceeds 0.3 (rut detection), penalize candidates whose parameter values
   are over-represented in recent successes. Fixes UQoR.

QSE addresses best_lat / best_area. QSD addresses UQoR.
Both keep SR comfortably above all baselines.
...
def stage_bambu_main():
    log('=' * 70)
    log(f'STAGE: Bambu PA-DSE+QSE+QSD '
        f'(γ={GAMMA_QSAT}, δ={DELTA_QSD})')
    log('=' * 70)
    ...
    for b in BAMBU_BENCH:
        ...
        for pid in range(N_PERMS):
            if is_done(done, 'PA-DSE_SCF+DFRL+QSE+QSD', b, BAMBU_BUDGET,
                       '', 'SCF+DFRL', str(pid)):
                continue
            log(f'{b} / PA-DSE+QSE+QSD / perm={pid}')
            try:
...
def stage_dynamatic_main():
    log('=' * 70)
    log(f'STAGE: Dynamatic PA-DSE+QSE+QSD '
        f'(γ={GAMMA_QSAT}, δ={DELTA_QSD})')
    log('=' * 70)
    ...
    for b in DYN_BENCH_PAPER:
        ...
        for pid in range(N_PERMS):
            if is_done(done, 'PA-DSE_SCF+DFRL+QSE+QSD', b, DYN_BUDGET,
                       '', 'SCF+DFRL', str(pid)):
                continue
            log(f'{b} / PA-DSE+QSE+QSD / perm={pid}')
            try:
...
def stage_ablation():
    ...
    for b in BAMBU_BENCH:
        ...
        for abl in ABLATIONS:
            for pid in range(3):
                if is_done(done, f'PA-DSE_{abl}+QSE+QSD', b, BAMBU_BUDGET,
                           '', abl, str(pid)):
                    continue
                log(f'{b} / {abl}+QSE+QSD / perm={pid}')
                try:
    ...
    for b in DYN_BENCH_PAPER:
        ...
        for abl in ABLATIONS:
            for pid in range(3):
                if is_done(done, f'PA-DSE_{abl}+QSE+QSD', b, DYN_BUDGET,
                           '', abl, str(pid)):
                    continue
                log(f'{b} / {abl}+QSE+QSD / perm={pid}')
                try:
...
def main():
    ...
    log(f'rerun_qse.py starting (γ={GAMMA_QSAT}, δ={DELTA_QSD})')
    log(f'cwd={os.getcwd()}')
